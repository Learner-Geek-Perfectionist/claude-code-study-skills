...
**每个 agent 的 prompt 必须包含以下信息（直接嵌入 prompt，不依赖外部文件）：**

1. `source_path_prefix` 的值（如 `../../`）
2. 交叉引用锚点映射表（完整表格）
3. 该模块对应的源码文件路径列表
4. 格式规范要点摘要（从 [format-rules.md](format-rules.md) 提取关键规则）
5. 章节编号和文件名
6. 输出路径：`{study_path}/{filename}.md`

**并行调度规则：**
- 在一条消息中使用多个 Agent 工具调用，同时启动所有剩余章节
- 每个 agent 独立运行，直接 Read 源文件进行分析
