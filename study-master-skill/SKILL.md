...
**规则**：所有源码必须位于 `src/`，所有协议/规范文档必须位于 `docs/`。如果两者都不存在，拒绝执行并提示：

```
❌ 错误：未找到 src/ 或 docs/ 目录
请确保源码位于 src/ 下，协议文档位于 docs/ 下
```
...
2. 按优先级查找：`src/<topic>/` → `src/*<topic>*/` → `docs/<topic>/` → `docs/*<topic>*/`（不区分大小写，支持部分匹配）
3. 找到多个候选时列出让用户选择
4. 输出目录：`study/<topic>/`
...
> 📖 格式标准详见 [format-rules.md](format-rules.md)
