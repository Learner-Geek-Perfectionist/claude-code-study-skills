...
>
> **源码路径前缀**：`{source_path_prefix}`（所有源码链接必须使用此前缀）
>
> **你负责的模块源文件**：{文件路径列表}
>
> **交叉引用锚点映射表**：
...
>
> **生成步骤**：
> 1. Read 上述源文件，理解模块的设计意图、核心函数、数据结构
> 2. 按教科书风格生成完整章节（先整体后局部、先接口后实现、先主流程后边界、先概念后代码）
> 3. 使用 Write 工具一次性输出到 `{study_path}/{filename}.md`
